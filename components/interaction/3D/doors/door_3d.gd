...
class_name IndieBlueprintDoor3D extends Node3D
...
@export var time_to_closdoore: float = 0.3
@export_group("AnimationPlayer")
@export var animation_player: AnimationPlayer
@export var open_door_animation: StringName = &"open"
@export var close_door_animation: StringName = &"close"
@export var locked_door_animation: StringName = &"locked"
@export var unlocked_door_animation: StringName = &"unlocked"

var door_tween: Tween

func _ready() -> void:
	if interactable == null:
		interactable = IndieBlueprintNodeTraversal.first_node_of_custom_class(self, IndieBlueprintInteractable3D) as IndieBlueprintInteractable3D
		
	if animation_player == null:
		animation_player = IndieBlueprintNodeTraversal.first_node_of_type(self, AnimationPlayer.new()) as AnimationPlayer
		
	assert(interactable is IndieBlueprintInteractable3D, "Door: The door %s needs an interactable to be valid" % door_name)
