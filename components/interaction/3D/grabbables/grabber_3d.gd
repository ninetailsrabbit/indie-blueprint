...
class_name IndieBlueprintGrabber3D extends Node3D

class ActiveGrabbable extends RefCounted:
	func _init(_grabbable: IndieBlueprintGrabbable3D, _slot: Node3D):
		body = _grabbable
		slot = _slot
		
	var body: IndieBlueprintGrabbable3D
	var slot: Node3D
	
	
signal pulled_grabbable(body: IndieBlueprintGrabbable3D)
signal throwed_grabbable(body: IndieBlueprintGrabbable3D)
signal dropped_grabbable(body: IndieBlueprintGrabbable3D)
...
@export var grabbable_interactor: IndieBlueprintGrabbableRayCastInteractor3D
...
@export_range(0.1, 100.0, 0.01) var grabbable_interactor_distance: float = 6.0:
	set(value):
		if grabbable_interactor is IndieBlueprintGrabbableRayCastInteractor3D \
...
func _input(_event: InputEvent) -> void:
	if push_wave_ability \
		...
		and IndieBlueprintInputHelper.action_just_pressed_and_exists(push_wave_input_action):
			
		push_wave()
		
	if pull_individual_ability and IndieBlueprintInputHelper.action_just_pressed_and_exists(pull_input_action):
		if slots_available():
			if grabbable_interactor and grabbable_interactor.is_colliding():
				var body = grabbable_interactor.get_collider() as IndieBlueprintGrabbable3D
				
				if body and not body.state_is_pull():
					pull_body(body)
	
	if pull_area_ability and IndieBlueprintInputHelper.action_just_pressed_and_exists(pull_area_input_action) and slots_available():
		var grabbables := get_near_grabbables()

		for body: IndieBlueprintGrabbable3D in grabbables:
			pull_body(body)
	...
	if IndieBlueprintInputHelper.action_just_pressed_and_exists(drop_input_action):
		for active_grabbable: ActiveGrabbable in active_grabbables:
			drop_body(active_grabbable.body)
			
	elif IndieBlueprintInputHelper.action_just_pressed_and_exists(throw_input_action):
		for active_grabbable: ActiveGrabbable in active_grabbables:
...
func pull_body(body: IndieBlueprintGrabbable3D, grabber: Node3D = get_random_free_slot()):
...
func pull_force(body: IndieBlueprintGrabbable3D):
	body.update_linear_velocity()
	body.update_angular_velocity()


func throw_body(body: IndieBlueprintGrabbable3D) -> void:
...
func drop_body(body: IndieBlueprintGrabbable3D) -> void:
	active_grabbables = active_grabbables.filter(func(active_grabbable: ActiveGrabbable): return active_grabbable.body != body)
	body.drop()
...
func push_wave():
	if push_wave_ability:
		var wave = IndieBlueprintPushWaveArea3D.new(IndieBlueprintCamera3DHelper.forward_direction(get_viewport().get_camera_3d()))
		add_child(wave)
		wave.activate()
...
func get_near_grabbables() -> Array:
	if grabbable_area_detector and grabbable_area_detector.monitoring:
		var bodies := grabbable_area_detector.get_overlapping_bodies().filter(func(body): return body is IndieBlueprintGrabbable3D)
		bodies.sort_custom(
			func(a: IndieBlueprintGrabbable3D, b: IndieBlueprintGrabbable3D): 
				return IndieBlueprintNodePositioner.global_distance_to_v3(a, self) <= IndieBlueprintNodePositioner.global_distance_to_v3(b, self))
...
func body_can_be_lifted(body: IndieBlueprintGrabbable3D) -> bool:
...
func _prepare_grabbable_area_detector():
	if grabbable_area_detector:
		...
		grabbable_area_detector.collision_mask = IndieBlueprintGameGlobals.grabbables_collision_layer


#region Signal callbacks
func on_dropped_grabbable(_grabbable: IndieBlueprintGrabbable3D) -> void:
	if grabbable_area_detector:
		grabbable_area_detector.monitoring = true


func on_throwed_grabbable(_grabbable: IndieBlueprintGrabbable3D) -> void:
	if grabbable_area_detector:
		grabbable_area_detector.monitoring = true
#endregion
