...
func items_with_weight_available(items: Array[LootItem] = available_items) -> Array[LootItem]:
	return items.filter(func(item: LootItem): return item.is_enabled and item.can_use_weight())


func items_with_rarity_available(items: Array[LootItem] = available_items) -> Array[LootItem]:
	return items.filter(func(item: LootItem): return item.is_enabled and item.can_use_rarity())


func items_with_valid_chance(items: Array[LootItem] = available_items) -> Array[LootItem]:
	return items.filter(func(item: LootItem): return item.is_enabled and item.can_use_chance())
