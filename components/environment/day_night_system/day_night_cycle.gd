...
@export var sun_color: Gradient = preload("res://components/environment/day_night_system/sun_gradients/normal/normal_sun_color_gradient.tres")
@export var sun_intensity: Curve = preload("res://components/environment/day_night_system/sun_gradients/normal/normal_sun_intensity_curve.tres")
...
@export var moon_color: Gradient = preload("res://components/environment/day_night_system/moon_gradients/normal/normal_moon_color_gradient.tres")
@export var moon_intensity: Curve = preload("res://components/environment/day_night_system/moon_gradients/normal/normal_moon_intensity_curve.tres")
...
@export var available_day_panoramas: Array[Texture2D] = [
	preload("res://components/environment/day_night_system/skybox/panoramas/dawn.png"),
	preload("res://components/environment/day_night_system/skybox/panoramas/day.png")
]
@export var available_dusk_panoramas: Array[Texture2D] = [
	preload("res://components/environment/day_night_system/skybox/panoramas/dusk.png"),
]

@export var overcast_panorama: Texture2D = preload("res://components/environment/day_night_system/skybox/panoramas/overcast.png")
@export var sky_top_color: Gradient = preload("res://components/environment/day_night_system/sky_color_gradients/normal/normal_sky_top_color_gradient.tres")
@export var sky_horizon_color: Gradient = preload("res://components/environment/day_night_system/sky_color_gradients/normal/normal_sky_horizon_gradient.tres")
@export var sky_ground_color: Gradient = preload("res://components/environment/day_night_system/sky_color_gradients/normal/normal_sky_ground_gradient.tres")
...
func _process(delta: float) -> void:
	...
	if Engine.get_process_frames() % 5 == 0:
		if use_global_day_night_clock:
			if GlobalDayNightClock.is_running:
				time = GlobalDayNightClock.curve_value
		else:
		...
		if sky_material:
			...
			sky_material.set_shader_parameter("sunColor", sun.light_color)
			sky_material.set_shader_parameter("sunBlur", time)
			sky_material.set_shader_parameter("sunDisc", clampf(time, min_sun_disc_size, max_sun_disc_size))
			sky_material.set_shader_parameter("skyColor", sky_top_color.sample(time))
			sky_material.set_shader_parameter("horizonColor", sky_horizon_color.sample(time))
			sky_material.set_shader_parameter("groundColor", sky_ground_color.sample(time))


func sun_visible() -> bool:
	if use_global_day_night_clock:
		return (is_dawn() or is_day()) and not ((GlobalDayNightClock.current_hour >= 18 and GlobalDayNightClock.current_minute >= 10))
...
func is_dawn() -> bool:
	if use_global_day_night_clock:
		return MathHelper.value_is_between(GlobalDayNightClock.current_hour, dawn_hour, day_hour - 1)
...
func is_day() -> bool:
	if use_global_day_night_clock:
		return MathHelper.value_is_between(GlobalDayNightClock.current_hour, day_hour, dusk_hour - 1)
...
func is_dusk() -> bool:
	if use_global_day_night_clock:
		return MathHelper.value_is_between(GlobalDayNightClock.current_hour, dusk_hour, night_hour - 1)
...
func is_night() -> bool:
	if use_global_day_night_clock:
		var current_hour = GlobalDayNightClock.current_hour
		
		return MathHelper.value_is_between(current_hour, night_hour, GlobalDayNightClock.DayHourLength - 1) \
			or MathHelper.value_is_between(current_hour, 0, dawn_hour)
...
func adjust_panorama_based_on_hour() -> void:
	if use_global_day_night_clock:
		if MathHelper.value_is_between(GlobalDayNightClock.current_hour, dusk_hour, night_hour, false):
...
func _connect_to_global_day_night_clock() -> void:
	if use_global_day_night_clock and is_inside_tree():
		if not GlobalDayNightClock.day_passed.is_connected(on_day_passed):
			GlobalDayNightClock.day_passed.connect(on_day_passed)
		
		if not GlobalDayNightClock.hour_passed.is_connected(on_hour_passed):
			GlobalDayNightClock.hour_passed.connect(on_hour_passed)
