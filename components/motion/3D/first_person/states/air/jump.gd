class_name FirstPersonJumpState extends FirstPersonAirState
...
var fall_detection_timer: Timer
...
func ready():
	...
	if fall_detection_timer == null:
		fall_detection_timer = IndieBlueprintTimeHelper.create_physics_timer(jump_fall_time, false, true)
		add_child(fall_detection_timer)
...
func apply_jump() -> void:
	...
	if actor.is_grounded:
		actor.footsteps_manager.footstep(0.4, FootstepSound.FootstepType.Jump)
...
func detect_fall_after_jump_fall_time_passed() -> void:
	if fall_detection_timer.time_left == 0:
		var up_direction_opposite: Vector3 = IndieBlueprintVectorHelper.up_direction_opposite_vector3(actor.up_direction)
