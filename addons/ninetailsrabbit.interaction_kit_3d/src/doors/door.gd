...
func _ready() -> void:
	if interactable == null:
		interactable = InteractionKit3DPluginUtilities.first_node_of_custom_class(self, Interactable3D) as Interactable3D
		
	if animation_player == null:
		animation_player = InteractionKit3DPluginUtilities.first_node_of_type(self, AnimationPlayer.new()) as AnimationPlayer
