...
func play(stream: AudioStream, volume: float = 1.0, bus: StringName = &"Master"):
...
func play_with_pitch(stream: AudioStream, volume: float = 1.0, pitch_scale: float = 1.0, bus: StringName = &"Master"):
	if _bus_is_valid(bus):
		var available_stream_player = _next_available_stream_player()
...
func play_with_pitch_range(stream: AudioStream, volume: float = 1.0, min_pitch_scale: float = 0.9, max_pitch_scale: float = 1.3, bus: StringName = &"Master"):
	if _bus_is_valid(bus):
		var available_stream_player = _next_available_stream_player()
...
func play_random_stream(streams: Array[AudioStream] = [], volume: float = 1.0, bus: StringName = &"Master"):
...
func play_random_stream_with_pitch(streams: Array[AudioStream] = [], volume: float = 1.0, pitch_scale: float = 1.0, bus: StringName = &"Master"):
	if streams.is_empty() or not _bus_is_valid(bus):
		return
...
func play_random_stream_with_pitch_range(streams: Array[AudioStream] = [], volume: float = 1.0, min_pitch_scale: float = 0.9, max_pitch_scale: float = 1.3,  bus: StringName = &"Master"):
	if streams.is_empty() or not _bus_is_valid(bus):
		return
...
func stop_streams_from_bus(bus: StringName = &"Master"):
...
func pause_streams_from_bus(bus: StringName = &"Master"):
	for player: AudioStreamPlayer in stream_players_pool:
		if player.bus.to_lower() == bus.to_lower():
...
func unpause_streams_from_bus(bus: StringName = &"Master"):
	for player: AudioStreamPlayer in stream_players_pool:
		if player.bus.to_lower() == bus.to_lower():
