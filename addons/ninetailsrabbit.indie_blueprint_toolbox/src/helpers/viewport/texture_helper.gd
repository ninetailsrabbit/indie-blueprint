...
static func get_png_rect_from_texture(texture: Texture2D) -> Rect2i:
	...
	assert(image !=null and image is Image, "IndieBlueprintTextureHelper->get_png_rect_from_texture: The image from the texture is null, the texture it's invalid")
