class_name SettingVSlider extends VSlider

@export var setting: GameSetting


func _enter_tree() -> void:
	assert(setting != null, "SettingVSlider: The %s hslider does not have a GameSetting resource linked" % name)
	assert(setting.field_type in [TYPE_INT, TYPE_FLOAT], "SettingVSlider: The '%s hslider' contains a setting '%s' that is not a int|float type" % [name, setting.key])
